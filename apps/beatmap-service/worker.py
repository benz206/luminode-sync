...
# Path to the streamrip config generated by entrypoint.sh.
_STREAMRIP_CONFIG_TEMPLATE = Path(os.path.expanduser("~/.config/streamrip/config.toml"))
...
def _write_job_config(tmpdir: str) -> str | None:
    """
    Copy the streamrip config template and patch the download folder to tmpdir.
    Returns the path to the per-job config, or None if the template is missing.

    streamrip v2 has no --directory flag; the output folder is config-only.
    We use `rip -c <config>` (top-level flag) to point at the per-job file.
    """
    if not _STREAMRIP_CONFIG_TEMPLATE.exists():
        log.warning("streamrip config template not found at %s", _STREAMRIP_CONFIG_TEMPLATE)
        return None
    try:
        content = _STREAMRIP_CONFIG_TEMPLATE.read_text()
        # Patch the 'folder' key to the job's temp dir.
        content = re.sub(r'(?m)^(folder\s*=\s*).*$', f'\\g<1>"{tmpdir}"', content)
        config_path = os.path.join(tmpdir, "streamrip-job.toml")
        Path(config_path).write_text(content)
        return config_path
    except Exception as exc:
        log.warning("Failed to write job streamrip config: %s — falling back to default", exc)
        return None
...
async def _process(job_id: str) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="beatmap-") as tmpdir:
            ...
            # Write a per-job config with 'folder' pointing at tmpdir — streamrip
            # v2 has no --directory CLI flag; the output path is config-only.
            log.info("[2/3] Downloading audio via streamrip")
            job_config = _write_job_config(tmpdir)
            rip_cmd = ["rip"]
            if job_config:
                rip_cmd += ["-c", job_config]
            rip_cmd += ["url", download_url]
            log.info("Running: %s", " ".join(rip_cmd))
